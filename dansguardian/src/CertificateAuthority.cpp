...
X509 * CertificateAuthority::generateCertificate(const char * commonname){
	...
	//from beginning of time (01-01-1970)
	if (!ASN1_TIME_set(X509_get_notBefore(newCert), 0)){
		X509_free(newCert);
		return NULL;
	}
	
	if(!ASN1_TIME_set(X509_get_notAfter(newCert), (long) LONG_MAX)){
		X509_free(newCert);
		return NULL;	
	}//to end of time (19-01-2038)
...
#ifdef DGDEBUG
	std::cout << "Searching for certificate for " << commonname << std::endl;
#endif
